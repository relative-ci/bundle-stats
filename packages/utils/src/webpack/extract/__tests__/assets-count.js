...
describe('Webpack/extract/extractAssetsCount', () => {
  test('should return empty', () => {
    const actual = extractAssetsCount(null, { metrics: { assets: {} } });
    expect(actual).toEqual({ metrics: { assetCount: { value: 0 } } });
  });

  test('should return size metrics', () => {
    const actual = extractAssetsCount(null, {
      metrics: {
        assets: {
          'js/vendor.min.js': {
    ...
    expect(actual).toEqual({ metrics: { assetCount: { value: 7 } } });
  });
});
