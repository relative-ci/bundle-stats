...
describe('Webpack/extract/extractAssetsChunkCount', () => {
  test('should return empty', () => {
    const actual = extractAssetsChunkCount(null, { metrics: { assets: {} } });
    expect(actual).toEqual({ metrics: { chunkCount: { value: 0 } } });
  });

  test('should return size metrics', () => {
    const actual = extractAssetsChunkCount(null, {
      metrics: {
        assets: {
          'js/vendor.min.js': {
    ...
    expect(actual).toEqual({ metrics: { chunkCount: { value: 3 } } });
  });
});
